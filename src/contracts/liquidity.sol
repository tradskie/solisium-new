...
contract Liquidity {
    ...
    function sweep() public returns (uint256){
        ...
        if (balanceOriginTokens >= 1e18  && tokenSupply_ > 0){
            ...
            //Add the new liquidity to SOLS dividends;
    ...
    /// @dev Distribute undividend in and out fees across SOLS pools and instant divs
    ...
    function purchaseTokens(address _customerAddress, uint256 _incomingtokens) internal returns (uint256) {
        ...
        uint256 _undividedDividends = SafeMath.mul(_incomingtokens, entryFee_) / 100;     // 10% of SLSL
        uint256 _amountOfTokens     = SafeMath.sub(_incomingtokens, _undividedDividends); // 90% of SLSL (100% - 10% above)
        ...
        //SOLS and buybacks; instant requires being called after supply is updated
        allocateFees(_undividedDividends);
